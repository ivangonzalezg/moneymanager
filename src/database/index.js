...
const getSalesByClient = () =>
  new Promise(resolve =>
    executeSql(
      `SELECT c.id AS id, c.name AS name, c.phone AS phone, SUM(CASE WHEN is_income = 1 THEN amount ELSE amount*-1 END) AS total FROM ${constants.tables.TRANSACTIONS} t JOIN ${constants.tables.CLIENTS} c ON t.client_id = c.id GROUP BY t.client_id ORDER BY name ASC`,
      [],
      (_, results) => resolve(results.rows.raw()),
      () => resolve([]),
    ),
